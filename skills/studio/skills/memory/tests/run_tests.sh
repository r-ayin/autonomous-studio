...
case "$url" in
  ...
  */smart-ingest*|*/smart-update*|*/update*|*/delete*)
    ...
    ;;
  */tree*)
    printf '{"status":"200","data":[{"id":1,"memoryId":"mem-1","name":"空间共识","path":"/空间共识/","nodeType":"folder","description":"AI 每次对话都要知道的协作空间背景","targetType":"workspace","targetId":"/空间共识/","tenant":"workspace_5","sortOrder":4,"parentId":null,"gmtCreate":"2026-05-25T06:13:37.000+00:00","gmtModified":"2026-05-25T06:13:37.000+00:00","prompt":null,"compressPrompt":null,"staffId":null,"repoId":null,"children":[{"id":2,"memoryId":"mem-2","name":"记忆","path":"/空间共识/记忆","nodeType":"memory","description":"保存协作空间背景","targetType":"workspace","tenant":"workspace_5"}]}]}'
    ;;
  *)
...
if ! echo "$out" | grep -q "space-memory search"; then
  ...
  pass "space-memory help lists workspace memory commands"
fi
...
if [[ "$code" -eq 0 ]]; then
  die "space-memory tree missing workspaceId should fail"
elif ! echo "$out" | grep -qi "usage"; then
  die "space-memory tree missing workspaceId should show usage"
else
  pass "space-memory tree missing workspaceId shows usage"
fi
