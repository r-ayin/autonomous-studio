...
- **`prdRef`**：该 task 对应的原始 PRD 章节锚点（格式 `planning/prd.md#章节标题`）。生成 prd.json 时必须为每个 task 填写，指向 prd.md 中描述该功能的段落。子 agent 在 task description 不够理解需求时，按此路径读取原始 PRD 获取完整上下文。
