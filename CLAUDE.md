...
### 🧠 Skill（`autonomous-studio/claude/skills/autonomous-engine/`）
| 文件 | 用途 |
|------|------|
| `SKILL.md` | 引擎调度器 Skill — 主会话激活入口 |
| `decision-agent-prompt.md` | 子 Agent 操作手册 — 独立上下文执行 |

### 📊 决策数据（`autonomous-studio/claude/decisions/`）
| 文件 | 用途 |
|------|------|
...
### 🧩 记忆系统（`autonomous-studio/claude/memory/`）
| 文件 | 用途 |
|------|------|
...
### 🔌 Hooks（`autonomous-studio/claude/hooks/`）
| 文件 | 触发时机 | 功能 |
|------|---------|------|
...
| 文件 | 用途 |
|------|------|
| `autonomous-studio/claude/decision-log.jsonl` | 决策日志 — `decision-observer.py` 写入 |
| `autonomous-studio/claude/checkpoints/latest.json` | 最新检查点 |
| `autonomous-studio/claude/sessions/{id}.json` | 会话上下文文件 |
| `autonomous-studio/claude/.watchdog_heartbeat` | 看门狗心跳 |
| `autonomous-studio/claude/watchdog.sh` | WSL 系统级 crontab（L6） |
...
```bash
# 查看引擎状态
cat autonomous-studio/claude/memory/autonomous-state.md

# 查看校准数据
cat autonomous-studio/claude/decisions/calibration.json | head -20

# 查看最近决策
tail -5 autonomous-studio/claude/decision-log.jsonl
