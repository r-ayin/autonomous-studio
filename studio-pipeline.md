...
**三条铁律：**
1. 讨论新要点前，先读一遍 `prd-decisions.md`，确认之前的结论还在
2. 每条共识格式：`- [x] 要点描述 | 结论 | 日期`
3. PRD 定稿时，逐条核对 `prd-decisions.md`，每条已确认的要点必须在 `prd.md` 中有明确体现，有遗漏必须指出并补回

**为什么这么做：** PRD 有 10 条要点，前 7 条确认后讨论后 3 条，之前的结论很容易被挤出上下文。写入文件是唯一可靠的保存方式。
...
- 生成前核对 `planning/prd-decisions.md`，确保每条 `[x]` 记录都在 prd.json 中有对应 task 覆盖
