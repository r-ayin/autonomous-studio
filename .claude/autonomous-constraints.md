...
## DO（审计指令 — 用户 2026-06-30 加，2026-07-01 重构为事件驱动 + 深度解绑，2026-07-08 加 E 前沿研究）
...
### E. 前沿研究调研（researching 状态，2026-07-08 加）

**为什么**：A 节全量审计是"内向代码审查"——只看本项目代码，不对比业界同类实现、不查论文。这导致引擎能发现 bug 但发现不了"功能落后于 SOTA"的结构性机会。E 节补这个盲区：周期性对外调研，把参考实现/论文里的更好方案转成可派发的 `research-informed` fix。

**触发条件**（事件驱动，复用 audit-cycle-state）：
1. `audit-cycle-state.status == cycle-complete` 时，看 `research_cadence.cycles_since_last_research >= research_every`（默认 2，即每完成 2 个审计周期触发 1 次研究）
2. 命中 → status 改 `researching`，`current_research_topic` 设为刚审完项目的核心功能（或 NEXT_SUGGESTION 里标记的"需调研"项），重置 `cycles_since_last_research=0`
3. 研究轮完成后 status → `fix-in-progress`（research-informed fix 走正常派发管线）

**研究轮 bounded 纪律**（防无界发散）：
- **单轮一个 topic**——不跨项目发散
- **WebSearch ≤8 次**——用 `mcp__builtin__web__WebSearch`（引擎：baidu/google/bing 三引擎可选），每次 query 聚焦一个子问题
- **开源实现搜 3-5 个**：GitHub/官方 repo，fetch README + 关键源码文件（用 mcp__builtin__web__WebFetch，github 走 raw.githubusercontent.com 优先）
- **论文搜 1-2 篇**：arxiv/scholar，fetch 摘要 + intro/conclusion
- **必须产出结构化对比表收敛**——不收敛则本轮失败，不空转重试
- **WebSearch 不可用**（403/超时连续 3 次）→ 降级为"基于训练知识对比"，report 标 `degraded: true`，仍产出对比表

**研究报告产出**：`.claude/research/research-YYYY-MM-DD-NNN.md`（不进 case JSON），含：
- `topic` / `our_current_approach`（引用本项目 file:line）
- 对比表：`| reference | key_idea | our_gap | applicability(high|med|low) | effort(S|M|L) | source_url |`
- `derived_research_fixes`：每个 applicability=high 的发现 → audit-cycle-state.derived_fixes 加一条 `{kind: research-informed, source: {type: oss|paper, name, url, reference_idea}, description, applicability, effort}`

**派生 research-informed fix**：
1. `kind=research-informed` 的 finding → 派生为重构/优化 case（通常 direction-shift，因参考实现常改公共接口）
2. case 的 `audit_findings[].kind` 写 `research-informed`，`audit_id` 写父研究 id（`research-YYYY-MM-DD-NNN`）
3. 派发、merge/reject 流程同 A 节 route-fix/direction-shift——复用现有 derived_fixes 管线，不新建

**DO NOT（研究轮禁止）**：
- 不直接 copy 开源代码（license 风险）——只借鉴思路，实现自己写
- 不为研究而研究——每个 derived fix 必须有明确 `our_gap` 证据（本项目 file:line 的不足）
- 不在研究轮做代码改动——研究轮只产出 report + derived_fixes，实现由后续派发轮做

---

关联 [[autonomous-engine-operations]]、[[smart-training-agent-skill-system-state]]、[[engine-foresight-research-phase]]。
