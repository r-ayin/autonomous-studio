...
MEMORY COMMANDS:
  ...
  space-memory <command>  Read/write workspace memories
...
COMMANDS:
  space-memory tree <workspaceId>
  space-memory search <workspaceId> <query> [memoriesPerNode] [totalLimit]
  space-memory add <workspaceId> <content> [--infer] [--target-path <path>]
  space-memory update <workspaceId> <id> <content>
  space-memory delete <workspaceId> <id>
  space-memory help
...
cmd_space_tree() {
  ...
  if [ -z "$workspace_id" ]; then
    error "Usage: space-memory tree <workspaceId>"
  fi
  require_no_extra_args "space-memory tree <workspaceId>" "$@"

  local response
  response=$(api_get_response "${WORKSPACE_BASE_URL}/spaces/${workspace_id}/tree")
  check_v2_response "$response"
  print_tree_response_data "$response"
}
...
cmd_space_add() {
  ...
  if [ -z "$workspace_id" ]; then
    error "Usage: space-memory add <workspaceId> <content> [--infer] [--target-path <path>]"
  fi
  ...
  local response
  response=$(api_post_json_response "${WORKSPACE_BASE_URL}/spaces/${workspace_id}/smart-ingest" "$json_body")
  check_v2_response "$response"
...
cmd_space_search() {
  ...
  if [ -z "$workspace_id" ] || [ -z "$query" ]; then
    error "Usage: space-memory search <workspaceId> <query> [memoriesPerNode] [totalLimit]"
  fi
  require_no_extra_args "space-memory search <workspaceId> <query> [memoriesPerNode] [totalLimit]" "$@"
  ...
  local response
  response=$(api_post_json_response "${WORKSPACE_BASE_URL}/spaces/${workspace_id}/smart-search" "$json_body")
  check_v2_response "$response"
  print_search_response_data "$response"
}
...
cmd_space_update() {
  ...
  if [ -z "$workspace_id" ] || [ -z "$id" ]; then
    error "Usage: space-memory update <workspaceId> <id> <content>"
  fi
  ...
  local response
  response=$(api_post_json_response "${WORKSPACE_BASE_URL}/spaces/${workspace_id}/smart-update" "$json_body")
  check_v2_response "$response"
...
cmd_space_delete() {
  ...
  if [ -z "$workspace_id" ] || [ -z "$id" ]; then
    error "Usage: space-memory delete <workspaceId> <id>"
  fi
  require_no_extra_args "space-memory delete <workspaceId> <id>" "$@"

  local json_body
  json_body=$(jq -n --arg i "$id" '{id: $i}')

  local response
  response=$(api_post_json_response "${WORKSPACE_BASE_URL}/spaces/${workspace_id}/delete" "$json_body")
  check_v2_response "$response"
