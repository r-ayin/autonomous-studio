...
| T1.9 | 用户明确指定 workspaceId 并要求写入协作空间记忆 | 使用 `space-memory add <workspaceId> ...` |
...
| T2.3 | 「按团队惯例处理这个发布」 | `user-memory search "团队惯例 发布"`，不要先问用户是否要查记忆 |
...
| T3.6 | 定向写入因目录/文件不存在失败 | 不继续猜路径 | 引导用户到 https://devix.alibaba-inc.com/devix/memory 新建或调整目录/文件；非强指定目录时可改自动路由 |
...
| C7.3 | `space-memory help` | 列出 tree/search/add/update/delete |
