...
5. 有 `planning/prd.md` 但无 `planning/prd.json` → `prd-review`（等待用户确认后生成 prd.json）
...
| ②a PRD | pm-spec | planning/prd.md + test-cases.md + prd.html → **等用户确认，不自动推进** |
| ②b 任务拆分 | （用户确认后） | planning/prd.json → 确认后才生成，不与 ②a 同步产出 |
...
### PRD 两步分离（强制）
1. **②a**：生成 prd.md + test-cases.md + prd.html，然后**停下等用户确认**
2. **②b**：用户明确确认 PRD 内容后，才从 prd.md 拆分生成 prd.json
3. 禁止在 ②a 阶段同时生成 prd.json
...
|---|---|
| 需求写完 | prd |
| PRD 写完（②a，等待确认） | prd-review |
| prd.json 生成完（②b） | development |
| 开发完成 | verification |
| 验证通过 | review |
| 评审通过 | deployment |
| 部署完成 | done |
