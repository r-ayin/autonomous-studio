...
**prd.json 驱动开发流程**（参考 ralph Agent 循环模式）：
...
7. Validator 通过 → 继续下一个 task
8. Validator 失败 → `notes` 写入失败原因，`status` 改回 `"pending"` → 开发者下轮自动读 notes 修复
9. 同一 task 连续失败 3 次 → `blocked: true`，跳过继续下一个
10. 所有 P0 tasks done → 运行 `python3 scripts/validate_prd_json.py` → 自动推进到 ⑤ 验证
