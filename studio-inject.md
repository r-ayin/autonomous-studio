...
### 列待办前先核验代码（强制）
凡是要向用户列出"待开发/待处理"清单，或报告某功能未完成时，**必须先拿任务描述里的功能点去代码库核验**：搜索对应的组件、函数、路由、UI 元素或数据字段，确认确实不存在，才能列为待办。
> 为什么：只看 tasks/ 或 planning/ 下有没有任务文件就判断"没做"，会把已实现的功能误报为待办，浪费用户信任。代码是唯一可信的现状来源，任务文件只是意图记录。
- 核验后发现已实现的：归档到 `archive/YYYY-MM-DD-{功能名}/`，**不删除**（保留意图与实现对照，供回顾）；归档后在原位置不留待办条目
- 核验后发现部分实现的：按"已完成 / 未完成"拆分，只把未完成部分列为待办，并注明已完成部分在哪
- 核验不确定的（如纯视觉问题需实跑页面）：明确标注"需实际打开页面确认"，不武断判定为待办
