...
# SCRIPTS-L03 fix (audit-2026-07-03-012): PROMPT 从单引号改 heredoc。
# 原因：单引号定义对未来编辑含 ' 字符（如 "don't"、"engine's"）脆弱，bash 解析会断；
# heredoc 对内容无转义要求，prompt 演进更安全。'<<'PROMPT_EOF'' 保留变量不展开语义。
read -r -d '' PROMPT <<'PROMPT_EOF' || true
你是 autonomous-studio 引擎的持续自治循环（Ralph Wiggum 模式，每轮新 context）。
当前轮次：推进持久化自动开发研究管线的**一个小工作单位**、**一次全量深度审计** 或 **一次前沿研究调研**（由 audit-cycle-state 决定）。

★ 预算：不设上限（用户 2026-06-27 指示）。普通轮/瞭望轮仍守小工作单位；**全量审计轮解绑深度限制**（可读 5-15 文件、追跨模块数据流、可用 sub-agent 但必须 model=sonnet）；**研究轮解绑联网**（用 mcp__builtin__web__WebSearch 搜开源实现 + 论文，但必须 bounded：单轮一个 topic、WebSearch ≤8 次、必须产出结构化对比表收敛）。

步骤：
0. 读 .claude/autonomous-constraints.md 全文（DO NOT 排除项 + DO 审计指令 A/B/C/D 四节）。然后读 .claude/memory/user-decision-style.md 全文（用户决策风格契约——执行前对齐用户沟通粒度、确认门槛、风险偏好、技术倾向；与约束文件同权重遵守；case 汇报写法和 outcome_evidence 须符风格，违反则按风格改写。文件不存在则跳过本步，但下轮建议加一条「P1：user-decision-style.md 缺失，应跑 distill-user-style.py 蒸馏」）。然后读 .claude/audit-cycle-state.json 决定**本轮类型**：
   - status=idle 或 cycle-complete → 先看 `research_cadence` 计数器（audit-cycle-state.json）：若 `cycles_since_last_research >= research_every`（默认 2）→ **本轮走前沿研究路径**（见下 status=researching 分支），重置计数器、设 status=researching + current_research_topic；否则计数器+1、走**全量审计路径**：选一个"未深度审过"的项目（看 .claude/audits/ 已有报告避免重复），深度审计（不限于 1-3 文件），产出独立 report 落 .claude/audits/audit-YYYY-MM-DD-NNN.md（不进 case JSON），把 findings 写到 audit-cycle-state.json 的 derived_fixes（每条标 kind: route-fix|direction-shift|structural|research-informed）。status 改 auditing→fix-in-progress。然后本轮结束（不派生 fix，下轮起逐个派生）。
   - status=researching → **本轮走前沿研究路径**（指令 E）：对 `current_research_topic`（通常是刚审完项目的核心功能）做 bounded 联网调研——用 mcp__builtin__web__WebSearch 搜 3-5 个同功能开源实现 + 1-2 篇相关论文（arxiv/scholar），逐个 fetch 关键源码/摘要，产出独立 report 落 .claude/research/research-YYYY-MM-DD-NNN.md（结构化对比表：{reference, key_idea, our_current_approach, gap, applicability, effort, source_url}）。把高适用性（applicability=high）的发现写成 derived_fixes（kind=research-informed，必填 source: {type: oss|paper, name, url, reference_idea}）。status 改 fix-in-progress（让 research-informed fix 走正常派发管线）。WebSearch 不可用（403/超时）时降级为"基于训练知识的对比"并在 report 里标 `degraded: true`，不空转重试。
...
5. 汇报 + 喂蒸馏闭环：做了什么 + 哪个 worktree + 下轮建议；并写一条 case 到 .claude/decisions/case-YYYY-MM-DD-NNN.json（字段：case_id/agent_id/timestamp/project/work_unit/situation/action/files_changed/worktree/direction/outcome[枚举 succeeded|failed|rolled_back|superseded|blocked]/outcome_evidence[引用可观察事实，不接受散文]/next_suggestion/**audit_type**[枚举 code-review|security-review|audit-log-instrumentation|deep-audit|research|none]/**audit_findings**[数组，每条 {file,line,severity,finding,remediation,kind}，无发现写 []]/**audit_id**[派生 fix 写父审计/研究 id，非审计 case 写 null]/**audit_depth**[枚举 shallow|deep|research，瞭望轮=shallow，全量审计轮=deep，研究轮=research]）。预算已解禁，case 要写实、写全。**派生 fix case 必须在 audit-cycle-state.json 的 derived_fixes 数组里同步 status=pending→merged|rejected**。
6. **回写状态**（持久化关键，否则目标无法跨 context 传递）：更新 .claude/memory/autonomous-state.md — GOAL_STATUS(active|paused|done)/ACTIVE_GOAL/LAST_UPDATED(YYYY-MM-DD)/LAST_WORKTREE/LAST_OUTCOME(done|blocked|in_progress)/NEXT_SUGGESTION(≥1 条下轮工作单位）。**同时回写 .claude/audit-cycle-state.json**（status/derived_fixes[].status/last_audit_*/research_cadence.cycles_since_last_research/current_research_topic/last_research_*），不写则审计/研究周期状态丢失。
...
PRIMARY_MODEL="qwen3.7-max"        # 2026-07-08 GLM-5.2 暂无额度，临时用 qwen3.7-max
FALLBACK_MODEL="qwen3.7-max"      # 兜底同主模型（GLM恢复后改回 GLM-5.2 兜底）
