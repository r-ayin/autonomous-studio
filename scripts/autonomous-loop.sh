...
1. 跑 python3 scripts/scout-scan.py --workspace '"$WORKSPACE"'（不带 --json，文本报告，轻）→ 拿项目健康快照（注：scout-scan.py 是 python 脚本带 #!/usr/bin/env python3 shebang + 可执行位，但不可用 `bash` 调用——bash 会把 python 语法当 shell 解析报 command not found；用 python3 调用或 ./scripts/scout-scan.py）
...
- 禁对 main checkout 直写主 ref：`git reset [--hard]` / `git update-ref` / `git branch -f|-d|-m` 针对主分支的写操作只许在 worktree 目录内跑（case-047 实证：CWD 漂移致 reset 在 main checkout 误执行，把 shizi main 快进 2 commit 绕过 commit-gate——gate 只拦 commit/push/merge 不拦 ref 直写）。须移 ref 时先 `cd` 进对应 .opt-worktrees/<proj>/<wt> 再操作，绝不依赖"当前 CWD 应该是 worktree"假设
...
# 连续 GLM 配额失败计数：≥3 则跳过 GLM 直接用 fallback，每 10 轮重试一次 GLM 探活
GLM_FAIL_STREAK=0
GLM_PROBE_INTERVAL=10

# 判断一轮输出是否为配额耗尽（429/402）
is_quota_error() {
  local out="$1"
  echo "$out" | grep -qE '429|402|"quota"|quota exceeded|balance insufficient|exceeded your current quota'
}
...
while true; do
  ...
  ROUND_OUT="$WORKSPACE/.claude/.loop-round-out"
  USE_MODEL="$PRIMARY_MODEL"
  # GLM 连续失败 ≥3：直接走 fallback，每 GLM_PROBE_INTERVAL 轮探活一次 GLM
  if (( GLM_FAIL_STREAK >= 3 )); then
    if (( ITER % GLM_PROBE_INTERVAL == 0 )); then
      echo "[$(date +%H:%M:%S)] GLM 连续失败 $GLM_FAIL_STREAK 次，本轮探活 GLM-5.2"
      USE_MODEL="$PRIMARY_MODEL"
    else
  ...
  echo "[$(date +%H:%M:%S)] 使用模型: $USE_MODEL"
  claude -p "$PROMPT" \
    --model "$USE_MODEL" \
    --permission-mode bypassPermissions 2>&1 | tee "$ROUND_OUT" | tail -30

  # 配额检测与回退
  if is_quota_error "$(cat "$ROUND_OUT" 2>/dev/null)"; then
    if [[ "$USE_MODEL" == "$PRIMARY_MODEL" ]]; then
      GLM_FAIL_STREAK=$((GLM_FAIL_STREAK+1))
      echo "[$(date +%H:%M:%S)] ⚠️ $USE_MODEL 配额耗尽(429/402)，连续 $GLM_FAIL_STREAK 次 → 回退 $FALLBACK_MODEL 重跑本轮"
      claude -p "$PROMPT" \
        --model "$FALLBACK_MODEL" \
        --permission-mode bypassPermissions 2>&1 | tee "$ROUND_OUT" | tail -30
      # fallback 也耗尽则记一笔，但不阻断下一轮
      if is_quota_error "$(cat "$ROUND_OUT" 2>/dev/null)"; then
        echo "[$(date +%H:%M:%S)] ⚠️ $FALLBACK_MODEL 也配额耗尽，本轮跳过，等下轮重试"
      fi
    fi
  else
    # 本轮成功（任何模型）→ 若是 GLM 成功则重置失败计数
    if [[ "$USE_MODEL" == "$PRIMARY_MODEL" ]]; then
      GLM_FAIL_STREAK=0
    fi
  fi
